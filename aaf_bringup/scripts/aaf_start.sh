...
tmux send-keys "DISPLAY=:0 roslaunch perception_people_launch people_tracker_robot.launch machine:=werner-left-cortex user:=strands with_wheelchair_detector:=true wheelchair_machine:=werner-left-cortex wheelchair_user:=strands with_tracker_filter_map:=true tracker_filter_map:=/opt/strands/map/y4_aaf-humans.yaml"
